...
export default function MembersScreen() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    'all' | 'active' | 'due-soon' | 'overdue' | 'paused'
  >('all');
  ...
  const statusMembers = filtered.filter(m => {
    if (statusFilter === 'all') return true;
    return memberStatus(m) === statusFilter;
  });

  const planMembers = filtered.filter(m => {
    if (planFilter === 'all') return true;
    return m.plan === planFilter;
  });

  const statusTotalPages = Math.ceil(statusMembers.length / pageSize);

  const planTotalPages = Math.ceil(planMembers.length / pageSize);

  const paginatedStatusMembers = statusMembers.slice(
    (statusPage - 1) * pageSize,
    statusPage * pageSize
  );

  const paginatedPlanMembers = planMembers.slice(
    (planPage - 1) * pageSize,
    planPage * pageSize
  );
          ...
          <div className="flex gap-2 flex-wrap">
            {FILTERS.filter(f =>
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl border text-[11px] font-medium tracking-[0.12em] uppercase transition-all ${statusFilter === f.id
          ...
          <div className="flex gap-2 flex-wrap">
            {FILTERS.filter(f =>
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl border text-[11px] font-medium tracking-[0.12em] uppercase transition-all ${planFilter === f.id
                    ? 'bg-[var(--accent-soft)] text-[var(--accent)] border-[color-mix(in_srgb,var(--accent)_35%,transparent)]'
                    : 'bg-[var(--bg)] text-[var(--text-dim)] border-[var(--border)] hover:text-[var(--text-1)] hover:border-[var(--border-strong)]'
                  }`}
                style={mono}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-1)] p-4">
          ...
          <div className="flex flex-col gap-2 min-h-[250px]">
            ...
            {paginatedStatusMembers.length === 0 && (
              <div className="flex items-center justify-center h-full text-[var(--text-mute)]">
                No members found
              </div>
            )}
          </div>
